...
SELECT
     Z.ID_ZBOR
    ,TRUNC(TO_NUMBER(Z.DATA_SOSIRE_REALA - Z.DATA_PLECARE_REALA) * 24*60) AS DURATA_ZBOR
    ,LEAST(0, TRUNC(TO_NUMBER(Z.DATA_SOSIRE_REALA - Z.DATA_SOSIRE_ESTIMATA) * 24*60)) AS INTARZIERE
    ,Z.ID_AVION
    ,Z.ID_PILOT
    ,Z.ID_COPILOT
...
JOIN DW.TIMP TS ON 
    EXTRACT(YEAR FROM Z.DATA_PLECARE_REALA) = TS.AN AND
    EXTRACT(MONTH FROM Z.DATA_PLECARE_REALA) = TS.LUNA AND
    EXTRACT(DAY FROM Z.DATA_PLECARE_REALA) = TS.ZI AND
    TO_CHAR(Z.DATA_PLECARE_REALA, 'HH24') = TS.ORA;

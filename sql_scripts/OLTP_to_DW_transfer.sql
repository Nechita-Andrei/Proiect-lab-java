...
AND AE.ID_AEROPORT NOT IN (SELECT ID_AEROPORT FROM DW.AEROPORT);
...
WHERE
    ...
    AND CA.ID_COMPANIE_AERIANA NOT IN (SELECT ID_COMPANIE_AERIANA FROM DW.COMPANIE_AERIANA);
...
SELECT
    ...
    ,ID_COMPANIE_AERIANA
FROM OLTP.AVION
WHERE ID_AVION NOT IN (SELECT ID_AVION FROM DW.AVION);
...
AND CL.ID_CLIENT NOT IN (SELECT ID_CLIENT FROM DW.CLIENT);
...
SELECT
    ...
    ,DESCRIERE
FROM OLTP.TIP_BILET
WHERE ID_TIP_BILET NOT IN (SELECT ID_TIP_BILET FROM DW.TIP_BILET);
...
SELECT
    ...
    ,ID_COMPANIE_AERIANA
FROM OLTP.PILOT
WHERE ID_PILOT NOT IN (SELECT ID_PILOT FROM DW.PILOT);
...
WHERE Z.ID_ZBOR NOT IN (SELECT ID_ZBOR FROM DW.ZBOR);
...
WHERE NOT EXISTS (
    SELECT 1 
    FROM DW.BILET DWB 
    WHERE DWB.ID_ZBOR = B.ID_ZBOR AND DWB.ID_CLIENT = B.ID_CLIENT
);